...
function parseExcel($excel_file_name_with_path)
	...
	$colname=array('NIS','NAMA_LENGKAP', 'NAMA_PANGGILAN', 'KELAMIN', 'TMP_LAHIR', 'TGL_LAHIR', 'AGAMA', 'WARGA_NEGARA', 'ANAK_KE',
	...
	if (empty($filex_namex))
		...
		if ($ext_filex == ".xls")
			...
			for($i=1;$i<$cprod;$i++)
				...
				$i_panggilan = addslashes($prod[$i][2]);

				//kelamin
				$i_kelamin = addslashes($prod[$i][3]);
				...
				$i_tmp_lahir = addslashes($prod[$i][4]);
				$i_tgl_lahir = titikdua(addslashes($prod[$i][5]));
				...
				$i_warga_negara = addslashes($prod[$i][7]);
				$i_anak_ke = addslashes($prod[$i][8]);
				$i_jml_sdr_kandung = addslashes($prod[$i][9]);
				$i_jml_sdr_tiri = addslashes($prod[$i][10]);
				$i_jml_sdr_angkat = addslashes($prod[$i][11]);
				$i_bahasa = addslashes($prod[$i][12]);



				$i_alamat = addslashes($prod[$i][13]);
				$i_telp = addslashes($prod[$i][14]);
				$i_jarak = addslashes($prod[$i][15]);



				$i_gol_darah = addslashes($prod[$i][16]);
				$i_penyakit = addslashes($prod[$i][17]);
				$i_kelainan = addslashes($prod[$i][18]);
				$i_tb = addslashes($prod[$i][19]);
				$i_bb = addslashes($prod[$i][20]);



				$i_tamat_dari = addslashes($prod[$i][21]);

				$i_tgl_ijazah = titikdua(addslashes($prod[$i][22]));
				...
				$i_no_ijazah = addslashes($prod[$i][23]);

				$i_lama_belajar = addslashes($prod[$i][24]);



				$i_diterima_kelas = addslashes($prod[$i][25]);

				$i_tgl_diterima = titikdua(addslashes($prod[$i][26]));
				...
				$i_ayah_nama = addslashes($prod[$i][27]);
				$i_ayah_tmp_lahir = addslashes($prod[$i][28]);

				$i_ayah_tgl_lahir = titikdua(addslashes($prod[$i][29]));
				...
				//agama
				$i_ayah_agama = addslashes($prod[$i][30]);
				...
				$i_ayah_warga_negara = addslashes($prod[$i][31]);
				$i_ayah_pendidikan = addslashes($prod[$i][32]);

				//pekerjaan
				$i_ayah_pekerjaan = addslashes($prod[$i][33]);
				...
				$i_ayah_penghasilan = addslashes($prod[$i][34]);
				$i_ayah_alamat = addslashes($prod[$i][35]);
				$i_ayah_telp = addslashes($prod[$i][36]);
				$i_ayah_hidup = addslashes($prod[$i][37]);



				//ibu
				$i_ibu_nama = addslashes($prod[$i][38]);
				$i_ibu_tmp_lahir = addslashes($prod[$i][39]);

				$i_ibu_tgl_lahir = titikdua(addslashes($prod[$i][40]));
				...
				//agama
				$i_ibu_agama = addslashes($prod[$i][41]);
				$qagmx = mysql_query("SELECT * FROM m_agama ".
				...
				$i_ibu_warga_negara = addslashes($prod[$i][42]);
				$i_ibu_pendidikan = addslashes($prod[$i][43]);

				//pekerjaan
				$i_ibu_pekerjaan = addslashes($prod[$i][44]);
				$qpekx = mysql_query("SELECT * FROM m_pekerjaan ".
				...
				$i_ibu_penghasilan = addslashes($prod[$i][45]);
				$i_ibu_alamat = addslashes($prod[$i][46]);
				$i_ibu_telp = addslashes($prod[$i][47]);
				$i_ibu_hidup = addslashes($prod[$i][48]);



				//wali
				$i_wali_nama = addslashes($prod[$i][49]);
				$i_wali_tmp_lahir = addslashes($prod[$i][50]);

				$i_wali_tgl_lahir = titikdua(addslashes($prod[$i][51]));
				...
				//agama
				$i_wali_agama = addslashes($prod[$i][52]);
				$qagmx = mysql_query("SELECT * FROM m_agama ".
				...
				$i_wali_warga_negara = addslashes($prod[$i][53]);
				$i_wali_pendidikan = addslashes($prod[$i][54]);

				//pekerjaan
				$i_wali_pekerjaan = addslashes($prod[$i][55]);
				$qpekx = mysql_query("SELECT * FROM m_pekerjaan ".
				...
				$i_wali_penghasilan = addslashes($prod[$i][56]);
				$i_wali_alamat = addslashes($prod[$i][57]);
				$i_wali_telp = addslashes($prod[$i][58]);





				$i_gemar_seni = addslashes($prod[$i][59]);
				$i_gemar_or = addslashes($prod[$i][60]);
				$i_gemar_lain = addslashes($prod[$i][61]);



				$i_bea_siswa = addslashes($prod[$i][62]);
				...
				//jika blm ada, insert
					...
					{
					//m_siswa
					mysql_query("INSERT INTO m_siswa(kd, usernamex, passwordx, nis, nama, panggilan, ".
									...
									"('$i_xyz', '$i_nis', '$i_pass', '$i_nis', '$i_nama', '$i_panggilan', ".
