...
if ($_POST['btnSMP'])
	...
	//nek null
	if ((empty($no)) OR (empty($no_sub)) OR (empty($pel)) OR (empty($xpel)) OR (empty($kkm)))
		{
		//diskonek
		xfree($qbw);
		xclose($koneksi);
		...
		if (empty($s))
			...
			if ($tcc != 0)
				...
				{
				//insert
				mysql_query("INSERT INTO m_mapel(kd, no, no_sub, pel, xpel, kkm, mulo) VALUES ".
								"('$x', '$no', '$no_sub', '$pel', '$xpel', '$kkm', '$mulo_ket')");
...
KKM : <input name="kkm" type="text" value="'.$kkm.'" size="2" maxlength="2">,
<br>
