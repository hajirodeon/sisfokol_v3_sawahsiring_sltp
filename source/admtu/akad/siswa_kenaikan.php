...
$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&skkd=$skkd&swkd=$swkd";
...
if (empty($tapelkd))
	...
	if (empty($rukd))
		...
		{
		$q_rukd = "AND siswa_kelas.kd_ruang = '$rukd' ";
		}
	...
	if ($count != 0)
		...
		do
			...
			if (($btxno == "1") OR ($btxno == "2"))
				...
				//terjemahkan ruang
				$qbf3 = mysql_query("SELECT * FROM m_ruang ".
							"WHERE kd = '$sk_rukd'");
				$rbf3 = mysql_fetch_assoc($qbf3);
				$bf3_ruang = balikin($rbf3['ruang']);


				if ($nuk_naik == "true")
					{
					$nuk_naik_ket = "<strong><font color=\"blue\">Naik Kelas : $bf2_kelas</font></strong>, Ruang : <strong>$bf3_ruang</strong>";
					}
				else if ($nuk_naik == "false")
					{
					$nuk_naik_ket = "<strong><font color=\"red\">Tinggal Kelas : $bf2_kelas</font></strong>, Ruang : <strong>$bf3_ruang</strong>";
					}
				else
					{
					$nuk_naik_ket = "-";
