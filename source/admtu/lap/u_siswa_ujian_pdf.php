...
require("../../inc/class/lap_u_siswa_ujian.php");
...
//query
$q = mysql_query("SELECT m_siswa.*, m_siswa.kd AS mskd, siswa_uang_ujian.*, ".
...
for ($we=0;$we<=($npage);$we++)
	...
	$qx = "SELECT m_siswa.*, m_siswa.kd AS mskd, siswa_uang_ujian.*, ".
			"DATE_FORMAT(siswa_uang_ujian.tgl_bayar, '%d') AS tgl, ".
			"DATE_FORMAT(siswa_uang_ujian.tgl_bayar, '%m') AS bln, ".
			"DATE_FORMAT(siswa_uang_ujian.tgl_bayar, '%Y') AS thn, ".
			"siswa_kelas.* ".
...
$pdf->WriteHTML($isi);
$pdf->Output("Keuangan_Siswa_Uang_Ujian.pdf",I);
?>