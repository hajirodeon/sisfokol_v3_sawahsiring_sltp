...
if ($_POST['btnSMP'])
	...
	//re-direct
	$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd&skkd=$skkd&swkd=$swkd&mapelkd=$mapelkd";
	xloc($ke);
	exit();
	}
...
do
	...
	echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&smtkd='.$st_kd.'&swkd='.$swkd.'&skkd='.$skkd.'">'.$st_smt.'</option>';
	}
...
if (empty($smtkd))
	...
	do
		...
		echo '<option value="'.$filenya.'?tapelkd='.$tapelkd.'&kelkd='.$kelkd.'&rukd='.$rukd.'&smtkd='.$smtkd.'&swkd='.$swkd.'&skkd='.$skkd.'&mapelkd='.$std_kd.'">'.$std_pel.'</option>';
		}
