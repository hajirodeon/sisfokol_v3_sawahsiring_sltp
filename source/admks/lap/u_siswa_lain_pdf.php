...
require("../../inc/class/lap_u_siswa_lain.php");
...
//query
$q = mysql_query("SELECT m_siswa.*, m_siswa.kd AS mskd, siswa_uang_lain.*, ".
...
for ($we=0;$we<=($npage);$we++)
	...
	$qx = "SELECT m_siswa.*, m_siswa.kd AS mskd, siswa_uang_lain.*, ".
				"DATE_FORMAT(siswa_uang_lain.tgl_bayar, '%d') AS tgl, ".
				"DATE_FORMAT(siswa_uang_lain.tgl_bayar, '%m') AS bln, ".
				"DATE_FORMAT(siswa_uang_lain.tgl_bayar, '%Y') AS thn, ".
				"siswa_kelas.* ".
...
$pdf->WriteHTML($isi);
$pdf->Output("Keuangan_Siswa_Uang_Lain.pdf",I);
?>