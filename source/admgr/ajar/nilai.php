...
if ($_POST['btnIM'])
	...
	if (empty($mapelkd))
		...
		{
		//re-direct
		$ke = "nilai.php?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd&mapelkd=$mapelkd&s=import";
		xloc($ke);
		exit();
		}
	}
...
if ($_POST['btnIM2'])
	...
	if (empty($filex_namex))
		...
		$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd&mapelkd=$mapelkd&s=import";
		pekem($pesan,$ke);
		}
	else
		{
		...
		if ($ext_filex == ".xls")
			...
			//re-direct
			$ke = "nilai_import.php?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd&mapelkd=$mapelkd&filex_namex=$filex_namex";
			xloc($ke);
			exit();
			}
		else
...
if ($_POST['btnEX'])
	...
	if (empty($mapelkd))
		...
		//re-direct
		$ke = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd&mapelkd=$mapelkd";
...
if (empty($smtkd))
	...
	if ($s == "import")
		...
		$pages = $p->findPages($count, $limit);
		$result = mysql_query("$sqlresult LIMIT ".$start.", ".$limit);
		$target = "$filenya?tapelkd=$tapelkd&kelkd=$kelkd&rukd=$rukd&smtkd=$smtkd&mapelkd=$mapelkd";
		$pagelist = $p->pageList($_GET['page'], $pages, $target);
		$data = mysql_fetch_array($result);
