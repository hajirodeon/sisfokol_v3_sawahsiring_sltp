...
if ($_POST['btnBTL'])
	...
	//re-direct
	$ke = "nilai.php?tapelkd=$tapelkd&kelkd=$kelkd&mapelkd=$mapelkd&rukd=$rukd&smtkd=$smtkd";
	xloc($ke);
	exit();
	}
...
if ($_POST['btnIMx'])
	...
	if (empty($filex_namex))
		...
		$ke = "nilai.php?tapelkd=$tapelkd&kelkd=$kelkd&mapelkd=$mapelkd&rukd=$rukd&smtkd=$smtkd&s=import";
		pekem($pesan,$ke);
		exit();
		}
	else
		...
		if ($ext_filex == ".xls")
			...
			for($i=1;$i<$cprod;$i++)
				...
				if ($tcc != 0)
					...
					if ($tcc1 != 0)
						{
						mysql_query("UPDATE siswa_nilai_mapel SET nh = '$i_nh', ".
								"tugas = '$i_tugas', ".
								"uts = '$i_uts', ".
								"uas = '$i_uas', ".
								"praktek = '$i_praktek', ".
